...
export function CommentList(props: CommentListProps) {
  ...
  } = useCommentQuery(GET_COMMENTS, {
    variables: { taskID: props?.taskID },
  });

  const [updateComment, { loading: commentUpdateLoading, error: commentUpdateError, data: commentUpdateData }] =
    useMutation(UPDATE_COMMENT, {
      refetchQueries: [{ query: GET_ALL_COMMENTS }],
    });
  const [deleteComment, { loading: commentDeleteLoading, error: commentDeleteError, data: commentDeleteData }] =
  ...
  useEffect(() => {
    ...
    if (!commentUpdateLoading && commentUpdateError) {
      getCommentErrorMessage(commentUpdateError?.graphQLErrors[0]?.extensions.exception);
    }
  ...
  useEffect(() => {
    ...
    if (!commentDeleteLoading && commentDeleteError) {
      getCommentErrorMessage(commentDeleteError?.graphQLErrors[0]?.extensions.exception);
    }
      ...
      {commentListData &&
        commentListData?.getComments
          ...
          .map((comment) => {
            ...
            return (
              <>
                {commentDeleteLoading || commentUpdateLoading ? (
                  <LazyLoading />
                ) : (
                          ...
                          {createdBy === loggedUserDetail.loggedUserName ? (
                            <div className="comments-action">
                              <span
                                ...
                                onClick={() => onClickEditComment(commentsID, commentDescription)}
                              >
                                <Icon name="pencil" />
                              </span>
                              <span className="checklist-actions" onClick={() => onClickDeleteComment(commentsID)}>
