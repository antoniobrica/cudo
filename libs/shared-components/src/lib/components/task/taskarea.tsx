...
export function TaskArea(props: Tasks) {
  ...
  const description = [<Segment>Pellentesque habitant morbi tristique senectus.</Segment>];
          ...
          {subtaskEditLoading ? (
            ...
            <>
              <span className="anchor_complete checklist-complete-box">
                <a title={t('project_tab_menu.task.completed')}>
                  {' '}
                  <span>
                    <i className="ms-Icon ms-Icon--Accept" aria-hidden="true"></i>
                  </span>{' '}
                </a>{' '}
              </span>
              <div className="classtop add-new-task-field">
                ...
                <Form.Field className="d-flex">
                  <button
                    ...
                    onClick={() => onClickSubTaskUpdate(props.task.taskID, subTaskId, subtaskTitle)}
                  >
                    <i className="ms-Icon ms-Icon--CheckMark" aria-hidden="true"></i>
                  </button>{' '}
                  &nbsp;{' '}
                  <button className="redbutton anchor_complete" onClick={onClickCancelEditSubTask}>
  ...
  const showSubTaskItems = (taskId, subTaskId, subTaskTitle, subtaskStatus, index) => {
    renderSubtaskItems = (
      <div className="d-flex align-items-center checklist-listing-main" key={subTaskId}>
        <span
          className="anchor_complete"
          onClick={() =>
            onClickSubTaskStatusUpdate(taskId, subTaskId, subtaskStatus === 'INPROGRESS' ? 'COMPLETED' : 'INPROGRESS')
          }
        >
        ...
        <span className="task-checklisting-text">
          {index + 1}. {subTaskTitle}
        </span>

        <span className="checklist-actions" onClick={() => onClickEditSubTask(taskId, subTaskId, subTaskTitle)}>
          {' '}
          <Icon name="pencil" />
        </span>
        <span className="checklist-actions" onClick={() => onClickDeleteSubTask(taskId, subTaskId)}>
  ...
  return (
    <div>
      {props?.task?.status === 'COMPLETED' ? (
          ...
          <div className="card-body">
            <div className="task-upper-con d-flex justify-content-between">
              <div className="d-flex align-items-center py-2">
                ...
                <span onClick={() => updateStatus(props.task, props.id)} className="anchor_complete">
                  {' '}
                  ...
                  <div className="navi navi-hover navi-active navi-link-rounded navi-bold d-flex flex-row task-listing-desc">
                    ...
                    <div className="navi-item">
                      <a className="navi-link">
                        <span className="navi-text">
                          {' '}
                          <i className="ms-Icon ms-Icon--CalendarAgenda" aria-hidden="true"></i> 5{' '}
                          {t('project_tab_menu.task.days')} <span className="dash-seperator">-</span>{' '}
                        </span>
                      </a>
                    </div>
                    {props?.task?.phaseName && (
                      <div className="navi-item">
                        <a className="navi-link">
                          <span className="navi-text">
                            {props?.task?.phaseName} <span className="dash-seperator">-</span>{' '}
                          </span>
                        </a>
                      </div>
                    )}
                      ...
                      subtaskData?.length > 0 ? (
                        <div className="navi-item">
                          <a className="navi-link">
                            <span className="navi-text">
                              {' '}
                              - &nbsp;{subtaskData?.length} {t('project_tab_menu.task.check_points')}{' '}
                            </span>
                          </a>
                        </div>
                      ) : null
                  ...
                  <div className="symbol symbol-30 d-flex">
                    ...
                    <span>
                      <Dropdown icon="ellipsis horizontal" pointing="right">
                        <Dropdown.Menu>
                          <Dropdown.Item
                            onClick={() => veiwTaskbyId(props.task, props.id)}
                            icon="eye"
                            text={t('common.view_details')}
                          />
                          <Dropdown.Item
                            onClick={() => editTaskbyId(props.task, props.id)}
                            icon="pencil"
                            text={t('common.edit')}
                          />
                          <Dropdown.Item
                            onClick={() => updateStatus(props.task, props.id)}
                            icon="check circle outline"
                            text={t('project_tab_menu.task.re_open')}
                          />
                          <Dropdown.Item
                            onClick={() => deleteTaskbyId(props.task, props.id)}
        ...
        <div
          className={
            props?.task?.taskType && props?.task?.taskType == 'PIN'
              ? 'card1 card-custom gutter-b task-main-con pinned-box'
              : 'card1 card-custom gutter-b task-main-con'
          }
        >
          <div className="card-body">
            <div className="task-upper-con d-flex justify-content-between">
              <div className="d-flex align-items-center py-2">
                ...
                <span className="textt">T-{props?.task?.sequenceNumber}</span>
                <span onClick={() => updateStatus(props.task, props.id)}>
                  <span className="anchor_complete">
                    <a title={t('project_tab_menu.task.completed')}>
                      {' '}
                      <span className="check-it-complete mr-2 mr-10">
                        <i className="ms-Icon ms-font-xl ms-Icon--Accept"></i>
                      </span>
                    </a>{' '}
                  </span>
                ...
                <div className="d-flex mr-3">
                  <div className="navi navi-hover navi-active navi-link-rounded navi-bold d-flex flex-row task-listing-desc">
                    {(props?.task.startDate || props?.task?.endDate) && (
                      <div className="navi-item">
                        <a className="navi-link active">
                          <span className="navi-text">
                            ( {props?.task?.startDate && new Date(props?.task?.startDate).toDateString()} ↦{' '}
                            {props?.task?.endDate && new Date(props?.task?.endDate).toDateString()})
                          </span>
                        </a>
                      </div>
                    )}
                      ...
                      <div className="navi-item">
                        <a className="navi-link">
                          <span className="navi-text">
                            ...
                            {props?.task?.estimatedDays} {t('project_tab_menu.task.days')}{' '}
                    ...
                    <div
                      className="navi-item"
                      onClick={() => (!isExpended ? openSubTask(props.task, props.id) : closeSubTask())}
                    >
                      <a className="navi-link active">
                        <span className="navi-text">
                ...
                {props.task?.assignees.length > 0 ? (
                  <div className="navi-item d-flex">
                    <a className="navi-link">
                      <span className="navi-text">
                        {' '}
                        <img src={`${MS_SERVICE_URL['ASSETS_CDN_URL'].url}/assets/images/user.png`} alt="" />{' '}
                      </span>
                    </a>
                    <Popup trigger={<Button className="more-user-listing">3+</Button>} flowing hoverable>
                      <Grid>
                      ...
                      <Dropdown icon="ellipsis horizontal" pointing="right">
                        <Dropdown.Menu className="dropdowncomplete">
                          ...
                          <Dropdown.Item
                            onClick={() => updateStatus(props.task, props.id)}
                            icon="check circle outline"
                            text={t('project_tab_menu.task.completed')}
                          />
                          <Dropdown.Item
                            onClick={() => deleteTaskbyId(props.task, props.id)}
                            icon="trash alternate outline"
            ...
            {isExpended && subtaskData && taskId === props.id ? (
              <div className="card1 card-custom gutter-b checklist-main-con">
                ...
                <div className="card-body">
                  {subtaskDeleteLoading && props.taskDeleteUpdateStatusLoading ? (
                    <LazyLoading />
                  ) : (
                    <div>
                      ...
                      {openSubTaskEdit === false && viewSubTaskAdd ? (
                            ...
                            <>
                              ...
                              <div className="classtop add-new-task-field">
                                ...
                                <Form.Field className="d-flex">
                                  <button
                                    className="greenbutton anchor_complete"
                                    onClick={() => createSubTask(props.task)}
                                  >
                                    <i className="ms-Icon ms-Icon--CheckMark" aria-hidden="true"></i>
                                  </button>{' '}
                                  &nbsp;{' '}
                        ...
                        <div onClick={addNewSubTask} className="add-new-link">
                          {' '}
                          <span className="anchor_complete">
                            <i className="ms-Icon ms-Icon--Add" aria-hidden="true"></i> {t('common.add_new_button')}
                          </span>
                        </div>
                      )}
                    </div>
