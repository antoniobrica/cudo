...
import { Button, Header, Modal, Input, Form, Grid, Select, TextArea, Dimmer, Loader } from 'semantic-ui-react';
...
export function ModalPlanningNew(props: PlanningProps) {
  ...
  React.useEffect(() => {
    if (props.worktypes) {
      setworkType(
        props.worktypes.map(({ workTypeName, projectWorkTypeID }) => ({
  ...
  return (
    <div>
      <Modal
        className={`modal_media right-side--fixed-modal add-new-milestone-modal${loader && ' overflow-hidden'}`}
        closeIcon
      ...
      >
              ...
              <Grid columns={2}>
                <Grid.Row>
                  ...
                  <Grid.Column>
                    <Form.Field>
                      <label>
                        {t('common.due_date')} <span className="danger">*</span>
                      </label>
                ...
                <Grid.Row>
                  <Grid.Column>
                    <Form.Field>
                      ...
                      <TextArea placeholder="Tell us more" value={description} onChange={onDescriptionChange} />
                      {/* <ReactQuill
                        value={description}
                        modules={{
                          toolbar: false
                ...
                <Grid.Row>
                  <Grid.Column>
                    <Form.Field>
                      ...
                      />
                      {errors?.workTypeError && !workTypeD ? (
              ...
              <Grid columns={1}>
                <Grid.Row>
                  <Grid.Column>
                    ...
                    <Form.Field>
                      <label>
                        {t('common.select_phase')} <span className="danger">*</span>
                      </label>
                      <PhaseIndex parentPhaseSelect={onsetPhasesID} error={errors?.phaseError && !phaseID} />
                      {errors?.phaseError && !phaseID ? (
                        <span className="error-message">{t('common.errors.phase_error')}</span>
                      ) : null}
                    </Form.Field>
                  </Grid.Column>
                </Grid.Row>
        ...
        <Modal.Actions>
          <Button content={t('common.submit')} onClick={createMilestone} positive size="small" className="primary" />
