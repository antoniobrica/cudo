...
export function CommentAdd(props: CommentAddProps) {
  ...
  const [addComment, { loading: commentAddLoading, error: commentAddError, data: commentAddData }] =
    ...
    addComment({
      variables: {
        parentUploadedFileID:
          props?.isVersionSelected === true ? props?.filesData?.parentUploadedFileID : props?.filesData?.uploadedFileID,
