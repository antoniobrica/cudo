...
export class BlobDownloadsViewStateService {
    ...
    return this.blobState.getStorageOptionsWithContainer().pipe(
      switchMap((options) =>
        this.blobStorage
          ...
          .pipe(this.getViewUrlFromResponse(), this.mapViewResponse(filename, options))
      )
    );
  };

  private mapDownloadResponse =
    (filename: string, options: BlobContainerRequest): OperatorFunction<string, BlobItemDownload> =>
    (source) =>
      source.pipe(
        map(
          (url) =>
  ...
  private getViewUrlFromResponse = (): OperatorFunction<BlobDownloadResponseModel, string> => (source) =>
    source.pipe(
      switchMap((res) =>
        from(res.blobBody ? res.blobBody : Promise.resolve({})).pipe(
          map((body) => {
