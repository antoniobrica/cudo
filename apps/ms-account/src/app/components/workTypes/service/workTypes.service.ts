...
export class WorkTypesService {
  ...
  public async updateWorkType(
    ...
    if (workType) {
      ...
      const updatedPost = await this.WorkTypeRepository.findOne({ where: { workTypeID: updateWorkType.workTypeID } });
      return updatedPost;
    }
  ...
  public async findAllWorkType(refFilter: ReferenceFilterParams): Promise<WorkTypeEntity[]> {
    ...
    return await this.WorkTypeRepository.find({ where: { reference: { id: selectedReference.id } } });
  }
