...
export class FileStructureService {
  ...
  public async createFileStructure(createFileStructureInput: CreateFileStructureInput, referenceFilter: ReferenceFilterParams): Promise<FileStructureEntity> {
    try {
  ...
  public async updateFileStructure(createFileStructureInput: CreateFileStructureInput, referenceFilter: ReferenceFilterParams): Promise<FileStructureEntity> {

    const selectedReference = await this.referenceService.getReferenceById(referenceFilter);
    const filestructure = await this.fileStructureRepository.findOne({ where: { fileStructureID: createFileStructureInput.fileStructureID, reference: { id: selectedReference.id } } });
    if (filestructure) {
      await this.fileStructureRepository.update(filestructure.id, { ...createFileStructureInput });
      const updatedPost = await this.fileStructureRepository.findOne(filestructure.id);
      return updatedPost;
    }
