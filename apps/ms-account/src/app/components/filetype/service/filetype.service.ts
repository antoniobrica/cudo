...
export class FileTypeService {
  ...
  public async createFileType(createfileTypeInput: CreateFileTypeInput, referenceFilter: ReferenceFilterParams): Promise<FileTypeEntity> {
    try {
  ...
  public async updateFileType(createfileTypeInput: CreateFileTypeInput, referenceFilter: ReferenceFilterParams): Promise<FileTypeEntity> {

    const selectedReference = await this.referenceService.getReferenceById(referenceFilter);
    const filetype = await this.FileTypeRepository.findOne({ where: { fileTypeID: createfileTypeInput.fileTypeID, reference: { id: selectedReference.id } } });
    if (filetype) {
      await this.FileTypeRepository.update(filetype.id, { ...createfileTypeInput });
      const updatedPost = await this.FileTypeRepository.findOne(filetype.id);
      return updatedPost;
    }
