...
export class MeetingCatagoryService {
  ...
  public async createMeetingCatagory(createInput: CreateMeetCatagoryInput, referenceFilter: ReferenceFilterParams): Promise<MeetingCatagoryEntity> {
    try {
  ...
  public async updateMeetingCatagory(updatefolder: UpdateMeetingCatagory, createinput: CreateMeetCatagoryInput): Promise<MeetingCatagoryEntity> {
    const catagory = await this.meetingCatagoryRepository.findOne({ where: { meetingCatagoryID: updatefolder.meetingCatagoryID } });
    if (catagory) {
      await this.meetingCatagoryRepository.update(catagory.id, { ...createinput });
      const updatedPost = await this.meetingCatagoryRepository.findOne(catagory.id);
      return updatedPost;
    }
