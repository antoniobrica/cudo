...
export class ReferenceService {
  ...
  async getReferenceById(refFilter: ReferenceFilterParams) {
    const reference = await this.referancesRepository.findOne({
      where: { ...refFilter },
      relations: ['tasks'],
    });
    if (reference) {
  ...
  async updateReference(
    ...
    referenceDetails: ReferenceUpdateInputDto
  ) {
    const reference = await this.referancesRepository.findOne({
      where: { ...refFilter },
    });
    if (reference) {
      await this.referancesRepository.update(reference.id, {
        ...referenceDetails,
      });
      const updatedPost = await this.referancesRepository.findOne({
  ...
  async deleteReference(refFilter: ReferenceFilterParams) {
    const { id } = await this.referancesRepository.findOne({
      where: { ...refFilter },
    });
