...
export class TasksService {
  ...
  public async create(
    createProjectTaskInput: TaskDetailsInput,
    referenceFilter: ReferenceFilterParams
  ): Promise<TasksEntity> {
    try {
      const { assignees, followers, files, taskBasics, subtasks } =
      ...
      if (assignees)
        for (let index = 0; index < assignees.length; index++) {
          ...
          const newAssignee = await this.tasksAssigneeRepository.create({
            ...assigneesentity,
          });
          const savedAssignee = await this.tasksAssigneeRepository.save(
            newAssignee
          );
      ...
      if (followers)
        for (let index = 0; index < followers.length; index++) {
          ...
          const newFollowers = await this.tasksFollowersRepository.create({
            ...followersentity,
          });
          const savedFollower = await this.tasksFollowersRepository.save(
            newFollowers
          );
      ...
      if (files)
        for (let index = 0; index < files.length; index++) {
          ...
          const newTaskFile = await this.taskFileRepository.create({
            ...taskfileEntity,
          });
      ...
      if (subtasks)
        for (let index = 0; index < subtasks.length; index++) {
          ...
          const newSubTask = await this.subTaskRepository.create({
            ...subtaskEntity,
          });
  ...
  public async updateTaskByID(
    createProjectTaskInput: TaskDetailsUpdateInput
  ): Promise<TasksEntity[]> {
    const { assignees, followers, files, taskBasics, subtasks } =
      createProjectTaskInput;
    ...
    taskBasics.estimatedDays
      ? (taskeDetail.estimatedDays = taskBasics.estimatedDays)
      : null;
    taskBasics.phaseID ? (taskeDetail.phaseID = taskBasics.phaseID) : null;
    taskBasics.phaseName
      ? (taskeDetail.phaseName = taskBasics.phaseName)
      : null;
    taskBasics.saveTaskAsTemplate
      ? (taskeDetail.saveTaskAsTemplate = taskBasics.saveTaskAsTemplate)
      : null;
    taskBasics.sendNotification
      ? (taskeDetail.sendNotification = taskBasics.sendNotification)
      : null;
    taskBasics.startDate
      ? (taskeDetail.startDate = taskBasics.startDate)
      : null;
    taskBasics.status ? (taskeDetail.status = taskBasics.status) : null;
    taskBasics.taskID ? (taskeDetail.taskID = taskBasics.taskID) : null;
    taskBasics.taskTitle
      ? (taskeDetail.taskTitle = taskBasics.taskTitle)
      : null;
  ...
  public async deleteTaskByID(
    ...
    const taskeDetails = await this.projectTasksRepository.delete({
      taskID: taskID,
    });
  ...
  public async deletesubTaskByID(
    subtaskDeleteInput: SubTaskFilterInput
  ): Promise<SubTaskEntity[]> {
    const { subtaskID } = subtaskDeleteInput;
    const subtaskeDetails = await this.subTaskRepository.delete({
      subtaskID: subtaskID,
    });
  ...
  public async updateSubTask(
    updateSubTask: SubTaskFilterInput,
    createinput: SubTaskInput
  ): Promise<SubTaskEntity> {
    const subtask = await this.subTaskRepository.findOne({
      where: { subtaskID: updateSubTask.subtaskID },
    });
    if (subtask) {
      await this.subTaskRepository.update(subtask.Id, { ...createinput });
      const updatedPost = await this.subTaskRepository.findOne({
        where: { subtaskID: subtask.Id.toString() },
      });
      return updatedPost;
    }
