...
export function ModalExampleModal(props: ProjectInfoProps) {
  ...
  const [addProject, { loading: addProjectLoading, error: addProjectError, data: addProjectData }] = useMutation(
    ADD_PROJECT
    // {
    //   refetchQueries: [
    //     { query: GET_PROJECTS }
  ...
  React.useEffect(() => {
    if (worktypeData) {
      setItems(
        worktypeData.workTypes.map(({ name, workTypeID }) => ({ key: name, value: name, text: name, id: workTypeID }))
      );
    }
  }, [worktypeData]);
  ...
  React.useEffect(() => {
    ...
    if (!addProjectLoading && !dataList && addProjectError) {
      props.getProjectErrorMessage(addProjectError.graphQLErrors[0]?.extensions.exception);
    }
  }, [addProjectLoading]);
  ...
  React.useEffect(() => {
    if (printingCompany) {
      setPrintingCompany(
        printingCompany.company.map(({ companyName }) => ({ key: companyName, value: companyName, text: companyName }))
      );
    }
  }, [printingCompany]);
  ...
  React.useEffect(() => {
    if (clientCompany) {
      setClientCompany(
        clientCompany.company.map(({ companyName }) => ({ key: companyName, value: companyName, text: companyName }))
      );
    }
  }, [clientCompany]);
        ...
        <Tab.Pane attached={false}>
          ...
          <div className="content upload-client-logo">
            <div className="description">
              {t('project_list.add_new_project.upload_logo_title')}{' '}
              <span>{t('project_list.add_new_project.upload_logo_line')}</span>
            </div>
            <Button className="secondary_btn" size="small" primary>
              {t('project_list.add_new_project.upload_logo_button')}
            </Button>
          </div>
            ...
            <Grid columns={2}>
              <Grid.Row>
                <Grid.Column>
                  ...
                  <Form.Field>
                    <label>
                      {t('project_list.add_new_project.project_name_label')}? <span className="danger">*</span>
                    </label>
                    <Input
                    ...
                    />
                    {errors?.titleError && !projectName ? (
                      <span className="error-message">{errors.titleError}</span>
                    ) : null}
                ...
                <Grid.Column>
                  <Form.Field>
                    <label>
                      {t('project_list.add_new_project.project_number_label')} <span className="danger">*</span>
                    </label>
                    <Input
                      placeholder="Default"
                      size="small"
                    ...
                    />
                    {errors?.numberError && !projectNum ? (
                      <span className="error-message">{errors.numberError}</span>
                    ) : null}
                  </Form.Field>
                </Grid.Column>
              </Grid.Row>
              ...
              <Grid.Row>
                <Grid.Column>
                  <Form.Field>
                    ...
                    />
                    {errors?.companyError && !client ? (
                      <span className="error-message">{errors.companyError}</span>
                    ) : null}
                  </Form.Field>
                  <Form.Field>
                ...
                <Grid.Column>
                  <Form.Field>
                    <label>
                      {t('project_list.add_new_project.building_type_lable')} <span className="danger">*</span>
                    </label>
                    <Select
                      placeholder={t('common.select')}
                      className="small"
                    ...
                    />
                    {errors?.buildingError && !buildingType ? (
                      <span className="error-message">{errors.buildingError}</span>
                    ) : null}
                  </Form.Field>
                </Grid.Column>
              </Grid.Row>
...
<Table.Row key={i}>
              ...
              <Grid.Row>
                <Grid.Column>
                  <Form.Field>
                    ...
                    <TextArea placeholder={t('common.tell_us_more')} value={description} onChange={onDescription} />
                    {/* <ReactQuill
                    value={description}
                    modules={{
                      toolbar: false
  ...
  return (
    <div className="add-project-area">
      {/* <div id="navbar"> */}
      <Modal
        className="modal_media right-side--fixed-modal add-new-project-modal"
        closeIcon
        ...
        trigger={
          <Button size="small" className="primary">
            <i className="ms-Icon ms-font-xl ms-Icon--Add ms-fontColor-themePrimary"></i> {t('common.add_new_button')}
          </Button>
        }
        closeOnDimmerClick={false}
      >
        <Modal.Header>
          <h3>{t('project_list.add_new_project.title')} </h3>
        </Modal.Header>
        <Modal.Content body>
          <div>
        ...
        <Modal
          className="right-side--fixed-modal add-new-project-modal"
          closeIcon
        ...
        >
          <Modal.Header>
            <h3>{t('project_list.add_new_project.add_company_title')}</h3>
          </Modal.Header>
          <Modal.Content body>
            <div>
              <div className="content">
                <div className="description">
                  {t('project_list.add_new_project.upload_logo_title')} <span className="danger">*</span>
                </div>
                  ...
                  <Grid.Row>
                    <Grid.Column>
                      <Form.Field>
                        ...
                        <Input
                          placeholder={t('project_list.add_new_project.email')}
                          size="small"
                          className="full-width"
                          type="text"
                        />
                  ...
                  <Grid.Row>
                    ...
                    <Grid.Column>
                      <Form.Field>
                        ...
                        <Input
                          placeholder={t('project_list.add_new_project.state_pin')}
                          size="small"
                          className="full-width"
                          type="text"
                        />
              ...
              <Button size="small" className="icon-border" onClick={() => setSecondOpen(false)}>
                <i className="ms-Icon ms-font-xl ms-Icon--CalculatorMultiply ms-fontColor-themePrimary"></i>{' '}
                {t('common.cancel')}
              </Button>
            </div>
          </Modal.Content>
