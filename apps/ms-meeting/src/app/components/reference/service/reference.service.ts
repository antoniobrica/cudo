...
export class ReferenceService {
    ...
    async getReferenceById(refFilter: ReferenceFilterParams) {
        const reference = await this.referancesRepository.findOne({ where: { ...refFilter }, relations: ['sessions'] });
        if (reference) {
            return reference;
        }
        throw new MeetingCustomError(MeetingErrorTypeEnum.PROJECT_NOT_FOUND)
    }
